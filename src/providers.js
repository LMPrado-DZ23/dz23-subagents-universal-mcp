...
function resolvedBase(name, d) {
  const direct = process.env[`${name.toUpperCase()}_BASE_URL`];
  if (direct) return direct;
  if (name === 'cloudflare') {
    const account = process.env.CLOUDFLARE_ACCOUNT_ID;
    return process.env.CLOUDFLARE_BASE_URL || (account ? `https://api.cloudflare.com/client/v4/accounts/${account}/ai/v1` : '');
  }
  return d.baseURL;
}
