...
async function boundedText(r,maxBytes=2*1024*1024){
  const length=Number(r.headers?.get?.('content-length')||0);if(length>maxBytes)throw new Error('Provider response exceeds configured limit');
  if(!r.body?.getReader){const text=await r.text();if(Buffer.byteLength(text)>maxBytes)throw new Error('Provider response exceeds configured limit');return text;}
  const reader=r.body.getReader();const decoder=new TextDecoder();let total=0,text='';
  while(true){const {done,value}=await reader.read();if(done)break;total+=value.byteLength;if(total>maxBytes){await reader.cancel();throw new Error('Provider response exceeds configured limit');}text+=decoder.decode(value,{stream:true});}
  return text+decoder.decode();
}
async function fetchText(url,opts,meta={}){const r=await fetch(url,opts);const text=await boundedText(r,meta.maxResponseBytes);if(!r.ok){const e=new Error(`${meta.name||'provider'}:${meta.model||''} HTTP ${r.status}: ${classifyHttpFailure(r.status,text)}`);e.status=r.status;e.kind=classifyHttpFailure(r.status,text);e.provider=meta.name;e.model=meta.model;e.retryAfterMs=retryAfterMs(r.headers);throw e;}return {r,text};}
export async function callOpenAICompatible(target,messages,{timeoutMs=90000,signal,temperature=0.2,maxTokens=4096,maxResponseBytes=2*1024*1024}={}){
  ...
  try{const headers={'content-type':'application/json'};if(target.apiKey&&target.apiKey!=='local')headers.authorization=`Bearer ${target.apiKey}`;const {text}=await fetchText(`${target.baseURL.replace(/\/$/,'')}/chat/completions`,{method:'POST',headers,signal:ctl.signal,body:JSON.stringify({model:target.model,messages,temperature,max_tokens:maxTokens})},{...target,maxResponseBytes});let data;try{data=JSON.parse(text);}catch{throw new Error(`Invalid JSON from ${target.name}`);}const content=data?.choices?.[0]?.message?.content;if(typeof content!=='string'||!content.trim())throw new Error(`No assistant content from ${target.name}`);return {content,usage:data.usage||null,raw:data};}finally{clearTimeout(timer);}
}
